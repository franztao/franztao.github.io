...
无论我们是单独工作还是与团队合作，重要的是我们有一个系统来跟踪我们项目的更改，以便我们可以恢复到以前的版本，以便其他人可以复制我们的工作并为它做出贡献。[Git](https://git-scm.com/)是一个分布式版本控制系统，它允许我们这样做。Git 在我们的计算机上本地运行，它会跟踪我们的文件及其历史记录。为了与他人协作，我们可以使用远程主机（[GitHub](https://github.com/)、[GitLab](https://gitlab.com/)、[BitBucket](https://bitbucket.org/)等）来托管我们的文件及其历史记录。我们将使用 git 推送我们的本地更改并将其他人的更改拉入和拉出远程主机。

> Git 传统上用于存储和版本化小于 100MB 的小文件（脚本、自述文件等），但是，我们仍然可以使用指向 blob 存储的文本指针来版本化大型工件（数据集、模型权重等）。[这些指针将包含诸如资产所在位置、特定内容/版本（例如通过散列）等信息。我们将在版本控制](https://madewithml.com/courses/mlops/versioning/)课程中看到这一点，我们将创建一个指向特定版本的指针我们的数据集和模型。
...
现在我们准备将更新从本地存储库推送到远程存储库。[首先在GitHub](https://github.com/) （或任何其他远程存储库）上创建一个帐户，然后按照[说明](https://github.com/new)创建一个远程存储库（它可以是私有的或公共的）。在我们的本地存储库中，我们将设置我们的用户名和电子邮件凭据，以便我们可以将更改从本地存储库推送到远程存储库。
...
现在我们准备开始添加到我们的项目并提交更改。

### 克隆

如果我们（或其他人）还没有设置本地存储库并与远程主机连接，我们可以使用克隆命令：
