...
2. `[Lines 12-35]`：定义不同的[处理程序](https://docs.python.org/3/library/logging.html#handler-objects)（有关发送日志消息的位置的详细信息）：
   ...
   - `info`：将日志消息（使用`detailed`格式化程序）发送到`logs/info.log`（一个可以达到的文件，`1 MB`我们将备份`10`它的最新版本）以获得级别以上的消息`INFO`。
   - `error`：将日志消息（使用`detailed`格式化程序）发送到`logs/error.log`（一个可以达到的文件，`1 MB`我们将备份`10`它的最新版本）以获得级别以上的消息`ERROR`。
3. `[Lines 36-40]`：将我们不同的处理程序附加到我们的根[Logger](https://docs.python.org/3/library/logging.html#logger-objects)。

我们选择使用字典来配置我们的记录器，但还有其他方法，例如 Python 脚本、配置文件等。单击下面的不同选项以展开并查看各自的实现。
...
我们使用[RichHandler](https://rich.readthedocs.io/en/stable/logging.html)作为我们的`console`处理程序来为日志消息获取漂亮的格式。这不是预安装的库，因此我们需要安装并添加到`requirements.txt`：

`pip install rich==12.4.4`

`# Add to requirements.txt rich==12.4.4`

我们记录的消息存储在日志目录中的相应文件中：
...
由于我们定义了详细的格式化程序，因此我们会看到如下信息丰富的日志消息：



## 应用

在我们的项目中，我们可以将所有打印语句替换为日志语句：
...
我们所有的日志消息都在`INFO`级别上，但在开发过程中我们可能不得不使用级别，如果我们的系统以意外的方式运行`DEBUG`，我们还会添加一些`ERROR`或日志消息。`CRITICAL`

- **what**：记录您希望从我们的应用程序中显示的所有必要详细信息，这些详细信息将在开发*期间*和*之后*的回顾性检查中有用。

- **其中**：最佳做法是不要将我们的模块化函数与日志语句混为一谈。相反，我们应该在小函数之外和更大的工作流程中记录消息。例如，除了`main.py`和`train.py`文件之外，我们的任何脚本中都没有日志消息。这是因为这些脚本使用了其他脚本（data.py、evaluate.py 等）中定义的较小函数。如果我们觉得我们需要在我们的其他功能中登录，那么它通常表明该功能需要进一步分解。

> [Elastic stack](https://www.elastic.co/what-is/elk-stack)（以前称为 ELK stack）是生产级日志记录的常用选项。它结合了[Elasticsearch](https://www.elastic.co/elasticsearch/)（分布式搜索引擎）、[Logstash](https://www.elastic.co/logstash)（摄取管道）和[Kibana](https://www.elastic.co/kibana)（可定制可视化）的特性。我们也可以简单地将我们的日志[上传](https://boto3.amazonaws.com/v1/documentation/api/latest/guide/s3-uploading-files.html)到云博客存储（例如 S3、谷歌云存储等）。
